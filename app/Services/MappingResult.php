...
class MappingResult
{
    ...
    public function formatForReference()
    {
        $part = $this->result;
    ...
    public function formatForMapping()
    {
        $part = $this->result;
        ...
        $this->result = collect([
            'status' => $merged_page['status'],
            'f' => array_count_values($merged_page['failures']->map(function($f) {
                return $f->failure_id;
            })
            ->toArray()),
            'm' => array_count_values($merged_page['modifications']->map(function($m) {
                return $m->modification->id;
            })
            ->toArray()),
            'h' => $merged_page['holes']->map(function($h) {
                $m = null;
                if ($h->holeModification->count() != 0) {
                    $m['id'] = $h->holeModification->first()->id;
                    $m['name'] = $h->holeModification->first()->name;
            ...
            ->toArray(),
            'hm' => array_count_values($merged_page['hModifications']->map(function($hm) {
                return $hm->m_id;
            })
            ->toArray()),
            'i' => $merged_page['inlines']->map(function($i) {
                return collect([
                    'id' => $i->id,
                    'sort' => $i->sort,
                    'max' => $i->max_tolerance,
